...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // GOOGLE AUTH LOGOUT/LOGIN WITH ROLE SELECT MODAL
    // ==========================================
    ...
    if (googleBtn) {
        googleBtn.addEventListener('click', (e) => {
            ...
            // Choose identity dialog
            const choice = prompt(
                "Continue with Google: Select your Google Identity:\n\n" +
                "1. Zenish Patel (zenish.patel@gmail.com)\n" +
                "2. Rajesh Patel (rajesh.supervisor@gmail.com)\n" +
                "3. Anjali Desai (anjali.teacher@gmail.com)\n" +
                "4. Admin Leader (admin.office@gmail.com)\n\n" +
                "Enter option (1-4):"
            );
            ...
            if (choice === '1') {
                email = 'zenish.patel@gmail.com';
                name = 'Zenish Patel';
            } else if (choice === '2') {
                email = 'rajesh.supervisor@gmail.com';
                name = 'Rajesh Patel';
            } else if (choice === '3') {
                email = 'anjali.teacher@gmail.com';
                name = 'Anjali Desai';
            } else if (choice === '4') {
                email = 'admin.office@gmail.com';
                name = 'Admin Leader';
            } else {
            ...
            // Save temporarily in local state variable
            googleTempUser = {
                id: 'USER-G-' + Date.now(),
                fullName: name,
                email: email,
            ...
            // Verify identity using SMTP OTP before displaying role selector modal
            const otpCode = Math.floor(100000 + Math.random() * 900000).toString();
            ...
            if (typeof emailjs !== 'undefined') {
                ...
                .then(() => {
                    const enteredOtp = prompt(
                        `[Google Security Verification: ${email}]\n\n` +
                    ...
                    if (enteredOtp === otpCode) {
                        // Open the modal only after successful OTP confirmation!
                        googleRoleModal.classList.add('active');
                    } else {
                        alert('Invalid verification code. Google sign-in aborted.');
                        googleTempUser = null;
                    }
                })
                .catch((error) => {
                    // EmailJS Error fallback
                    console.error('EmailJS Error:', error);
                    const enteredOtp = prompt(
                        `[Google Security Fallback (EmailJS Error: ${error.text || error})]\n\n` +
                        `We could not send the email verification code.\n` +
                        `Verification Code: ${otpCode}\n\n` +
                        `Enter the 6-Digit code below to bypass:`
                    );
                ...
                // Offline fallback
                const enteredOtp = prompt(
                    `[Google Security Verification - Offline Fallback]\n\n` +
                    `An OTP has been requested for your Google login identity: ${email}\n\n` +
                    `Verification Code: ${otpCode}\n\n` +
                    `Enter the 6-Digit code to confirm login:`
                );
    ...
    // Submit modal chosen role selection
    if (modalContinueBtn) {
        modalContinueBtn.addEventListener('click', () => {
            ...
            if (!googleTempUser) {
                alert('Identity credentials lost. Please trigger Google Login again.');
                return;
            }
            ...
            // Seed user record in local database if not present
            const users = JSON.parse(localStorage.getItem('users') || '[]');
            ...
            if (!userObj) {
                ...
                // Update role in local array as requested by prompt ("Save selected role to profile")
                userObj.role = chosenRole;
                localStorage.setItem('users', JSON.stringify(users));
            }

            // Close modal overlay & commit session
            googleRoleModal.classList.remove('active');
            localStorage.setItem('currentUser', JSON.stringify(userObj));
            
            alert(`Logged in with Google as ${userObj.fullName} (${chosenRole.toUpperCase()})!`);
